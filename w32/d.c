...
#if WIN32||_WIN64 
ZS mb(S s){R MessageBox(0,s,"kdb+",MB_OK|MB_TASKMODAL),s;}
Z HINSTANCE hi;BOOL APIENTRY DllMain(HANDLE h,DWORD i,LPVOID l){if(i==DLL_PROCESS_ATTACH)hi=h;R 1;}
BOOL CALLBACK cbc(HWND h,UINT m,WPARAM w,LPARAM l){if(m==WM_COMMAND)SW(w){
 ...
 *s=0,c=khpu(q,atoi(s+1),u&&*u?(strncpy(b,u,j=sj(u,uj)),u=b+j,*u++=':',j=sj(p,pj),strncpy(u,p,j)[j]=0,b):(S)""),*s=':';Q(c<0,q)Q(!c,"access")R n=1,0;}
A SQLDriverConnect(D d,HWND w,S s,H j0,S r,H j,H*j2,UH f){
 ...
 if(r){if(strcpy(r,"DRIVER=kdb+;"),av(r,"DBQ",q),*u)av(r,"UID",u),av(r,"PWD",p);av(r,"DSN","default"),*j2=strlen(r);}R SQLConnect(d,q,S0,u,S0,p,S0);}

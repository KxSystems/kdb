...
K eval(K x,K y,K z){K*k;S*b,s;SQLULEN w;SQLLEN*nb;SQLINTEGER*wb;H*tb,u,t,j=0,p,m,ubi=0,bi=0;F f;J v;C c[128];I n=xj<0;D d=d1(n?-xj:xj);U(d)x=y;Q(z->t!=-KJ||xt!=-KS&&xt!=KC,(S)"type")
 ...
 for(;SQL_SUCCEEDED(SQLFetch(d));)DO(j,k=kK(y)+i;u=ut[t=tb[i]];k=kK(y)+i;u=ut[t=tb[i]];if(!bi&&wb[i])QC(SQLGetData(d,(H)i+1,ct[t],b[i],wb[i],nb+i),CLEAN(j));s=b[i];n=SQL_NULL_DATA==(int)nb[i];if(!u)jk(k,n?ktn(ct[t]?KC:KG,0):wb[i]?kp(s):gb(d,(H)(i+1),t));else ja(k,n?nu(u):u==KH&&wb[i]==1?(t=(H)*s,(S)&t):u==KS?(s=dtb(s,nb[i]),(S)&s):u==2?na(4,s),na(2,s+4),na(2,s+6),s:u==KN?(v=ns(s),(S)&v):u==KP?(v=ps(s),(S)&v):u<KD?s:u==KZ?(f=ds(s)+(vs(s+6)+*(I*)(s+12)/1e9)/8.64e4,(S)&f):(w=u==KD?ds(s):vs(s),(S)&w)))

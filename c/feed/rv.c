...
Z M mq(K x){S s;K y,z;I j;M m,t;if(y=xt==XD?*xK:0){U(y->t==KS)x=xK[1];}if(r1(x),j=0<xt)x=k(0,"k)*:'(::),(;0)'",x,(K)0);
 U(!xt)tibrvMsg_Create(&m);DO(xn-j,z=xK[i+j];s=y?kS(y)[i]:0;
  if(z->t==XD){U(t=mq(z))tibrvMsg_AddMsg(m,s,t);md(t);}else{f.name=s;U(fq(z))tibrvMsg_AddField(m,&f);})R r0(x),m;}
ZK snd(I i,S s,K x){M m=mq(x),r;P(!m,krr("type"))tibrvMsg_SetSendSubject(m,s);P(i<0,(tibrvTransport_Send(-i,m),md(m),(K)0))

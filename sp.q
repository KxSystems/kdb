...
/sql)select p.color,sum(sp.qty) from sp,p where sp.p=p.p group by p.color order by color

select from sp where s.city=p.city
/sql)select sp.s,sp.p,sp.qty from s,p,sp where sp.s=s.s and sp.p=p.p and p.city=s.city

select distinct p,s.city from sp
/sql)select distinct sp.p,s.city from sp,s where sp.s=s.s

...
\timing on
explain(analyze)select sym,last(bid) from quote where sym in(select sym from S)group by sym;
explain(analyze)select sym,ex,max(price) from trade where sym in(select sym from S)group by sym,ex;
explain(analyze)select sym,extract(hour from time),avg(size) from trade where sym in(select sym from S)group by sym,extract(hour from time);
...
\timing on
copy trade from '/a/tq/t.csv' delimiter ',' csv;create index ts on trade(sym);
copy quote from '/a/tq/q.csv' delimiter ',' csv;create index qs on quote(sym);
analyze;

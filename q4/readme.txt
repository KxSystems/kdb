TRILLION ROW BENCHMARKS ftp://ftp.nyxdata.com/Historical%20Data%20Samples/Daily%20TAQ/

k&q are 10 to 100 times faster than other colstore(vertical, big3accel, hadoop/impala/parquet, ..)
k&q are 100 to 1000 times faster than the rowstore(postgres, big3rdbms, mongodb, spark, ..)
k&q scale out best(lowest overhead. fastest messaging. lowest RAM.) see http://en.wikipedia.org/wiki/Fallacies_of_distributed_computing
...
product: k5, q3, big3rdbms, big3accel, postgres, hadoop/impala/parquet, mongodb, vertical, spark/shark, ..
queries: 3 aggregations on top 100 symbols (1/4th of data) plus an asof-join regnms (price<current bid).
...
Q4: select time,price,bid from aj[`time;select from trade where date=d,sym=`QQQ;select from quote where date=d,sym=`QQQ]where price<bid

SMALL day (40Million rows) times in milliseconds. e.g. q3 Query3 is 230 times faster than spark/shark.
                Q1      Q2      Q3      Q4   RAM(GB)    ETL   DSK(GB) OVERHEAD(ms)
k5               1       9       8       2      .1       12      1         .001
q3              17      12      36       3      .2       40      1         .01
vertical       500     140     150    8900     2.1       52       .5      4.0
hadoop/impala 4800    1190    1000     DNF     4.0       22       .3    100.0
big3accel     4200    1600    2300     DNF     3.4       20      1         .3
postgres      7100    1500    1900     DNF     1.5      200      4         .1
big3rdbms     6400    2200    3100     DNF     5.0       60      2         .3
mongodb       8900    1700    5800     DNF     9.0      922     10         .1
spark/shark  34000    7400    8400     DNF    50.0      156      2.4       .1                          
...
OVERHEAD is milliseconds for fast query, e.g. q3 can do 100,000 different queries per second per cpu.

LARGE day  (640Million rows)
k5               1      35      40       8
q3             290      33     130      29
...
SCALE(k5) (1-1600 days:   1Trillion rows)  (>20GB per second of selected data)
    1 day:       1      35      40       8    /          .6Billion rows
   16 day:      10      60      70      17    /        10  Billion rows
   32 day:      17      83     100      20    /        20  Billion rows
 1600 day:     750    3500    5000    1000    /     1,000  Billion rows 

all database products index by sym so the number of rows scanned should be about 25Billion. (2 or 3 columns)
...
sql
 select sym,bid from (select sym,bid,row_number()over(partition by sym order by time desc)as row from quote where date=d and sym in(select sym from S))q where row=1;
 select sym,ex,max(price) from trade where date=d and sym in(select sym from S)group by sym,ex;
 select sym,hour(time),avg(size) from trade where date=d and sym in(select sym from S)group by sym,hour(time);
...
we can duplicate these timings using:
http://kx.com/q/bench/gen.k

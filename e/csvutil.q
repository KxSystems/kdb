...
info0:{[file;onlycols]
	...
	info:update t:"M",rule:90,maybe:1b from info where t="I",mw=6,.csv.cancast["M"]peach sdv; / 200506, YYYYMM is less likely than [H]HMMSS so do that first 
	...
	info:update t:"F",rule:120,maybe:0b from info where t="n",mdot<2,mw>1,{all x in".+-eE0123456789"}each dchar,.csv.cancast["F"]peach sdv;
	info:update t:"E",rule:130,maybe:0b from info where t="F",mw<8,.csv.cancast["E"]peach sdv; / need to check for "1e40" etc
	info:update t:"M",rule:140,maybe:1b from info where t="E",mw=7,.csv.cancast["M"]peach sdv; / 2005.06 
	info:update t:"M",rule:150,maybe:0b from info where t="n",mw=7,mdot=0,.csv.cancast["M"]peach sdv; / 2005/06 2005-06
	info:update t:"D",rule:160,maybe:0b from info where t="n",mw in 6 7 8 10,mdot in 0 2,.csv.cancast["D"]peach sdv; / 2005.06.07 2005/06/07 2005-06-07
	info:update t:"D",rule:170,maybe:1b from info where t="I",mw=8,.csv.cancast["D"]peach sdv; / 20050607  
	info:update t:"D",rule:180,maybe:0b from info where t="?",mw in 7 9 11,mdot in 0 2,.csv.cancast["D"]peach sdv; / 29oct2005 29oct05 etc
  ...
  info:update t:"T",rule:201,maybe:1b from info where t="n",mw within 7 10,mdot=1,{all x like"*[0-9][0-5][0-9][0-5][0-9].*"}peach sdv;
	...
	info:update t:"Z",rule:220,maybe:0b from info where t in"n?",mw within 11 24,mdot<4,{$[all x in"0123456789.:ABCDEFGJLMNOPRSTUVYabcdefgjlmnoprstuvy/- ";1<sum".:/ -T"in x;0b]}each dchar,.csv.cancast["Z"]peach sdv;
	...
	info:update t:"B",rule:260,maybe:1b from info where t in"?IHC",mw=1,mdot=0,{all x in"01tTfFyYnN"}each dchar; 
	...
	info:update t:"S",rule:280,maybe:1b from info where t="?",mw<.csv.SYMMAXWIDTH,mw>1; / symbols (max width permitting)

...
info0:{[file;onlycols;extended]
  ...
  info:update t:"N",(rules:rules,'53),maybe:1b from info where extended,t="n",mw=15,mdot=0,{all x in"0123456789"}each dchar,.csv.cancast["N"]peach sdv; / N, could be T but that'd loose precision
  info:update t:"T",(rules:rules,'54),maybe:1b from info where extended,t="n",mw=9,mdot=0,{all x in"0123456789"}each dchar,.csv.cancast["T"]peach sdv;
  ...
  info:update t:"M",(rules:rules,'110),maybe:1b from info where extended,t in"nIHEF",mdot<2,mw within 4 7,.csv.cancast["M"]peach sdv;
  info:update t:"D",(rules:rules,'120),maybe:1b from info where extended,t="I",mw in 6 8,.csv.cancast["D"]peach sdv;
  info:update t:"D",(rules:rules,'121),maybe:0b from info where t="n",mdot=0,mw within 8 10,.csv.cancast["D"]peach sdv;
  info:update t:"D",(rules:rules,'122),maybe:0b from info where t="n",mdot=2,mw within 8 10,.csv.cancast["D"]peach sdv;
  info:update t:"V",(rules:rules,'130),maybe:1b from info where extended,t="I",mw=6,{all x like"[012][0-9][0-5][0-9][0-5][0-9]"}peach sdv,.csv.nostar["V"]peach sdv; / 235959 123456
  info:update t:"U",(rules:rules,'140),maybe:1b from info where extended,t="H",mw=4,{all x like"[012][0-9][0-5][0-9]"}peach sdv,.csv.nostar["U"]peach sdv; /2359
...
infolike:{[file;pattern] info0[file;{x where(lower x)like lower y}[colhdrs[file];pattern];1b]} / .csv.infolike[file;"*time"]
infoonly:info0[;;1b] / only some columns .csv.infoonly[file;`this`and`that] 
